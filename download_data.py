...
class Downloader:
    ...
    def download(
        ...
        print(
            f"[Before cross-ckeck] Number of files in the training set: {len(os.listdir(self.root_path + '/train/'))}")
        print(
            f"[Before cross-ckeck] Number of files in the validation set: {len(os.listdir(self.root_path + '/val/'))}")
    ...
    def cross_check(self):
        ...
        print(
            f"Difference between the original training set and the updated one: {len(self.train_df) - len(self.updated_train_df)}")
        print(
            f"Difference between the original validation set and the updated one: {len(self.val_df) - len(self.updated_val_df)}")
        print(
            f"Difference between the original test set and the updated one: {len(self.test_df) - len(self.updated_test_df)}")
    ...
    def download_file(
        ...
        os.system(
            f'yt-dlp -x --audio-format {self.format} --audio-quality {self.quality} --output "{target_file_path}" --postprocessor-args "-ss {start_seconds} -to {end_seconds}" https://www.youtube.com/watch?v={ytid}')

        return

d = Downloader(root_path='/data/wei/audiocaps/', n_jobs=16)
