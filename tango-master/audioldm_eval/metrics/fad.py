...
class FrechetAudioDistance:
    def __init__(
        self, use_pca=False, use_activation=False, verbose=False, audio_load_worker=8, device='cuda:0'
    ):
        self.__get_model(use_pca=use_pca, use_activation=use_activation, device=device)
        self.verbose = verbose
        self.audio_load_worker = audio_load_worker
    def __get_model(self, use_pca=False, use_activation=False, device='cuda:0'):
        """
        Params:
        -- x   : Either
        ...
        self.model = VGGish(device=device)
        if not use_pca:
