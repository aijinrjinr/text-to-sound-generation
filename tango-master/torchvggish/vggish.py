...
class VGGish(VGG):
    def __init__(self, device=None, pretrained=True, preprocess=True, postprocess=True, progress=True):
        super().__init__(make_layers())
        if pretrained:
            # state_dict = hub.load_state_dict_from_url(urls['vggish'], progress=progress)
            state_dict = torch.load('/home/qywei/.cache/torch/hub/checkpoints/vggish-10086976.pth')#, progress=progress)
        ...
        if self.postprocess:
            ...
            if pretrained:
                state_dict = torch.load('/home/qywei/.cache/torch/hub/checkpoints/vggish_pca_params-970ea276.pth')#, progress=progress)
